...
Alert._prefix = "dsn_"; // Change this if you have your own solution prefix (as long as the file structure's the same)
...
Alert._jQuery = thisWindow[jqId] ? thisWindow[jqDolarId] : thisWindow[parentId][jqDolarId];
...
Alert.show = function (title, message, buttons, icon, width, height, baseUrl, preventCancel, padding) {
    ...
    if (!Alert._initialised) {
        ...
        if (alertJsWrapper == null || alertJsWrapper.length == 0) {
            var alertJsHtml = "<div id='alertJs-wrapper' class='alert-js-wrapper'>" +
    ...
    if (title == "") {
        Alert.$("#alertJs-title").hide();
    }
    else {
        Alert.$("#alertJs-title").show();
    }
    // Hide message if not specified
    if (message == "") {
        Alert.$("#alertJs-message").hide();
    }
    else {
        Alert.$("#alertJs-message").show();
    }
    // Add the icon
    ...
    if (preventCancel) {
        Alert.$("#alertJs-closeWrapper").hide();
    }
    else {
        Alert.$("#alertJs-closeWrapper").show();
    }
    // Makes the formatting nicer if the popup is huge (for displaying trace logs etc)
    if (height > 250) {
        Alert.$(".alert-js-td").addClass("alert-js-td-top");
    }
    else {
        Alert.$(".alert-js-td").removeClass("alert-js-td-top");
    }
